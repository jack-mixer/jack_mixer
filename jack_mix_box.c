...
void usage() {
  ...
  printf(
   "-v|--volume\tinitial volume gain in dB, default is 0.0 (i.e. unity "
   "gain)\n");
  printf("\n");
  printf(
   "Each positional argument is interpreted as a MIDI Control Change number "
   "and adds\n");
  printf(
   "a mixer channel with one (mono) or left+right (stereo) inputs, whose "
   "volume can\n");
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    static struct option long_options[] = {{"name", required_argument, 0, 'n'},
     {"help", no_argument, 0, 'h'}, {"stereo", no_argument, 0, 's'},
     {"volume", required_argument, 0, 'v'}, {0, 0, 0, 0}};
    int option_index = 0;
