...
static jack_mixer_output_channel_t create_output_channel(
 jack_mixer_t mixer, const char *channel_name, bool stereo, bool system);

static inline void update_channel_buffers(
 struct channel *channel_ptr, jack_nframes_t nframes);
...
float db_to_value(float db) { return powf(10.0, db / 20.0); }
...
double interpolate(double start, double end, int step, int steps) {
  ...
  if (start <= 0) {
    if (step <= frac * steps) {
      ...
      ret =
       db_to_value(value_to_db(frac * end) +
                   (value_to_db(end) - value_to_db(frac * end)) * step / steps);
      ;
    }
  } else if (end <= 0) {
    if (step >= (1 - frac) * steps) {
      ...
      ret = db_to_value(
       value_to_db(start) +
       (value_to_db(frac * start) - value_to_db(start)) * step / steps);
    }
  } else {
    ret = db_to_value(value_to_db(start) +
                      (value_to_db(end) - value_to_db(start)) * step / steps);
  }
...
void channel_rename(jack_mixer_channel_t channel, const char *name) {
  ...
  if (channel_ptr->stereo) {
    ...
    ret = jack_port_rename(
     channel_ptr->mixer_ptr->jack_client, channel_ptr->port_left, port_name);
    if (ret != 0) {
    ...
    ret = jack_port_rename(
     channel_ptr->mixer_ptr->jack_client, channel_ptr->port_right, port_name);
    if (ret != 0) {
      /* what could we do here? */
    }
    ...
    ret = jack_port_rename(
     channel_ptr->mixer_ptr->jack_client, channel_ptr->port_left, name);
    if (ret != 0) {
      /* what could we do here? */
    }
  }
...
int channel_get_balance_midi_cc(jack_mixer_channel_t channel) {
  return channel_ptr->midi_cc_balance_index;
}

static void channel_unset_midi_cc_map(
 jack_mixer_channel_t channel, int new_cc) {
  if (channel_ptr->mixer_ptr->midi_cc_map[new_cc]->midi_cc_volume_index ==
      new_cc) {
    channel_ptr->mixer_ptr->midi_cc_map[new_cc]->midi_cc_volume_index = -1;
  } else if (channel_ptr->mixer_ptr->midi_cc_map[new_cc]
              ->midi_cc_balance_index == new_cc) {
    channel_ptr->mixer_ptr->midi_cc_map[new_cc]->midi_cc_balance_index = -1;
  } else if (channel_ptr->mixer_ptr->midi_cc_map[new_cc]->midi_cc_mute_index ==
             new_cc) {
    channel_ptr->mixer_ptr->midi_cc_map[new_cc]->midi_cc_mute_index = -1;
  } else if (channel_ptr->mixer_ptr->midi_cc_map[new_cc]->midi_cc_solo_index ==
             new_cc) {
    channel_ptr->mixer_ptr->midi_cc_map[new_cc]->midi_cc_solo_index = -1;
  }
}

unsigned int channel_set_balance_midi_cc(
 jack_mixer_channel_t channel, int new_cc) {
  ...
  if (channel_ptr->midi_cc_balance_index != -1) {
    channel_ptr->mixer_ptr->midi_cc_map[channel_ptr->midi_cc_balance_index] =
     NULL;
  }
...
int channel_get_volume_midi_cc(jack_mixer_channel_t channel) {
  return channel_ptr->midi_cc_volume_index;
}

unsigned int channel_set_volume_midi_cc(
 jack_mixer_channel_t channel, int new_cc) {
  if (new_cc < 0 || new_cc > 127) {
    return 2; /* error: outside limit CC */
  }
  ...
  if (channel_ptr->midi_cc_volume_index != -1) {
    channel_ptr->mixer_ptr->midi_cc_map[channel_ptr->midi_cc_volume_index] =
     NULL;
  }
  channel_ptr->mixer_ptr->midi_cc_map[new_cc] = channel_ptr;
...
int channel_get_mute_midi_cc(jack_mixer_channel_t channel) {
  return channel_ptr->midi_cc_mute_index;
}

unsigned int channel_set_mute_midi_cc(
 jack_mixer_channel_t channel, int new_cc) {
  if (new_cc < 0 || new_cc > 127) {
    return 2; /* error: outside limit CC */
  }
...
int channel_get_solo_midi_cc(jack_mixer_channel_t channel) {
  return channel_ptr->midi_cc_solo_index;
}

void channel_set_midi_cc_volume_picked_up(
 jack_mixer_channel_t channel, bool status) {
  LOG_DEBUG(
   "Setting channel %s volume picked up to %d", channel_ptr->name, status);
...
void channel_set_midi_cc_balance_picked_up(
 jack_mixer_channel_t channel, bool status) {
  LOG_DEBUG(
   "Setting channel %s balance picked up to %d", channel_ptr->name, status);
  channel_ptr->midi_cc_balance_picked_up = status;
}

unsigned int channel_set_solo_midi_cc(
 jack_mixer_channel_t channel, int new_cc) {
  if (new_cc < 0 || new_cc > 127) {
    return 2; /* error: outside limit CC */
  }
...
int channel_autoset_volume_midi_cc(jack_mixer_channel_t channel) {
  ...
  for (int i = 11; i < 128; i++) {
    if (mixer_ptr->midi_cc_map[i] == NULL) {
      ...
      LOG_DEBUG(
       "New channel \"%s\" volume mapped to CC#%i", channel_ptr->name, i);
...
int channel_autoset_balance_midi_cc(jack_mixer_channel_t channel) {
  struct jack_mixer *mixer_ptr = channel_ptr->mixer_ptr;
  ...
  for (int i = 11; i < 128; i++) {
    if (mixer_ptr->midi_cc_map[i] == NULL) {
      ...
      LOG_DEBUG(
       "New channel \"%s\" balance mapped to CC#%i", channel_ptr->name, i);
...
int channel_autoset_mute_midi_cc(jack_mixer_channel_t channel) {
  struct jack_mixer *mixer_ptr = channel_ptr->mixer_ptr;
  ...
  for (int i = 11; i < 128; i++) {
    if (mixer_ptr->midi_cc_map[i] == NULL) {
      ...
      LOG_DEBUG(
       "New channel \"%s\" mute mapped to CC#%i", channel_ptr->name, i);
...
int channel_autoset_solo_midi_cc(jack_mixer_channel_t channel) {
  struct jack_mixer *mixer_ptr = channel_ptr->mixer_ptr;
  ...
  for (int i = 11; i < 128; i++) {
    if (mixer_ptr->midi_cc_map[i] == NULL) {
      ...
      LOG_DEBUG(
       "New channel \"%s\" solo mapped to CC#%i", channel_ptr->name, i);
...
void remove_channel(jack_mixer_channel_t channel) {
  GSList *list_ptr;
  channel_ptr->mixer_ptr->input_channels_list =
   g_slist_remove(channel_ptr->mixer_ptr->input_channels_list, channel_ptr);
  free(channel_ptr->name);

  /* remove references to input channel from all output channels */
  for (list_ptr = channel_ptr->mixer_ptr->output_channels_list; list_ptr;
  ...
  jack_port_unregister(
   channel_ptr->mixer_ptr->jack_client, channel_ptr->port_left);
  if (channel_ptr->stereo) {
    jack_port_unregister(
     channel_ptr->mixer_ptr->jack_client, channel_ptr->port_right);
  }
  ...
  if (channel_ptr->midi_cc_mute_index != -1) {
    assert(
     channel_ptr->mixer_ptr->midi_cc_map[channel_ptr->midi_cc_mute_index] ==
     channel_ptr);
    channel_ptr->mixer_ptr->midi_cc_map[channel_ptr->midi_cc_mute_index] = NULL;
  }
  if (channel_ptr->midi_cc_solo_index != -1) {
    assert(
     channel_ptr->mixer_ptr->midi_cc_map[channel_ptr->midi_cc_solo_index] ==
     channel_ptr);
    channel_ptr->mixer_ptr->midi_cc_map[channel_ptr->midi_cc_solo_index] = NULL;
  }
...
void channel_stereo_meter_read(
 jack_mixer_channel_t channel, double *left_ptr, double *right_ptr) {
  assert(channel_ptr);
...
void channel_stereo_kmeter_read(jack_mixer_channel_t channel, double *left_ptr,
 double *right_ptr, double *left_rms_ptr, double *right_rms_ptr) {
  assert(channel_ptr);
...
void channel_mono_kmeter_read(
 jack_mixer_channel_t channel, double *mono_ptr, double *mono_rms_ptr) {
...
void channel_volume_write(jack_mixer_channel_t channel, double volume) {
  ...
  if (channel_ptr->volume_new != channel_ptr->volume) {
    channel_ptr->volume =
     interpolate(channel_ptr->volume, channel_ptr->volume_new,
      channel_ptr->volume_idx, channel_ptr->num_volume_transition_steps);
  }
...
double channel_volume_read(jack_mixer_channel_t channel) {
  assert(channel_ptr);
  return value_to_db(channel_ptr->volume_new);
}

void channels_volumes_read(jack_mixer_t mixer_ptr) {
  GSList *node_ptr;
  struct channel *pChannel;
  struct jack_mixer *pMixer = (struct jack_mixer *)mixer_ptr;

  for (node_ptr = pMixer->input_channels_list; node_ptr;
...
void channel_balance_write(jack_mixer_channel_t channel, double balance) {
  ...
  if (channel_ptr->balance != channel_ptr->balance_new) {
    channel_ptr->balance =
     channel_ptr->balance + channel_ptr->balance_idx *
...
double channel_balance_read(jack_mixer_channel_t channel) {
  assert(channel_ptr);
...
void channel_out_mute(jack_mixer_channel_t channel) {
  channel_ptr->out_mute = true;
  channel_ptr->midi_out_has_events |= CHANNEL_MUTE;
}

void channel_out_unmute(jack_mixer_channel_t channel) {
  channel_ptr->out_mute = false;
  channel_ptr->midi_out_has_events |= CHANNEL_MUTE;
}

bool channel_is_out_muted(jack_mixer_channel_t channel) {
...
void channel_solo(jack_mixer_channel_t channel) {
  ...
  channel_ptr->mixer_ptr->soloed_channels =
   g_slist_prepend(channel_ptr->mixer_ptr->soloed_channels, channel);
...
void channel_unsolo(jack_mixer_channel_t channel) {
  if (g_slist_find(channel_ptr->mixer_ptr->soloed_channels, channel) == NULL)
    return;
  channel_ptr->mixer_ptr->soloed_channels =
   g_slist_remove(channel_ptr->mixer_ptr->soloed_channels, channel);
  channel_ptr->midi_out_has_events |= CHANNEL_SOLO;
}
...
void channel_set_midi_scale(
 jack_mixer_channel_t channel, jack_mixer_scale_t scale) {
  channel_ptr->midi_scale = scale;
}

void channel_set_midi_change_callback(jack_mixer_channel_t channel,
 void (*midi_change_callback)(void *), void *user_data) {
  channel_ptr->midi_change_callback = midi_change_callback;
  channel_ptr->midi_change_callback_data = user_data;
}

bool channel_get_midi_in_got_events(jack_mixer_channel_t channel) {
...
static inline void mix_one(struct output_channel *output_mix_channel,
 ...
 jack_nframes_t end)   /* index of sample to stop processing before */
{
  ...
  for (i = start; i < end; i++) {
    ...
    if (mix_channel->stereo)
      mix_channel->right_buffer_ptr[i] =
       mix_channel->tmp_mixed_frames_right[i] = 0.0;
  }
  ...
  for (node_ptr = channels_list; node_ptr; node_ptr = g_slist_next(node_ptr)) {
    ...
    if (g_slist_find(output_mix_channel->muted_channels, channel_ptr) != NULL ||
        channel_ptr->out_mute) {
      /* skip muted channels */
      continue;
    }

    if ((!channel_ptr->mixer_ptr->soloed_channels &&
         !output_mix_channel->soloed_channels) ||
        (channel_ptr->mixer_ptr->soloed_channels &&
         g_slist_find(channel_ptr->mixer_ptr->soloed_channels, channel_ptr) !=
          NULL) ||
        (output_mix_channel->soloed_channels &&
         g_slist_find(output_mix_channel->soloed_channels, channel_ptr) !=
          NULL)) {
      for (i = start; i < end; i++) {
        if (!output_mix_channel->prefader &&
            g_slist_find(output_mix_channel->prefader_channels, channel_ptr) ==
             NULL) {
          frame_left = channel_ptr->frames_left[i - start];
        } else {
          frame_left = channel_ptr->prefader_frames_left[i - start];
        }
        if (frame_left == NAN) break;
        mix_channel->tmp_mixed_frames_left[i] += frame_left;
        if (mix_channel->stereo) {
          if (!output_mix_channel->prefader &&
              g_slist_find(
               output_mix_channel->prefader_channels, channel_ptr) == NULL) {
            frame_right = channel_ptr->frames_right[i - start];
          } else {
            frame_right = channel_ptr->prefader_frames_right[i - start];
          }
          if (frame_right == NAN) break;
  ...
  for (i = start; i < end; i++) {
    if (!output_mix_channel->prefader) {
      ...
      if (balance != balance_new) {
        bal =
         mix_channel->balance_idx * (balance_new - balance) / steps + balance;
      }
    ...
    if (!mix_channel->out_mute) {
      ...
      if (mix_channel->stereo)
        mix_channel->right_buffer_ptr[i] =
         mix_channel->tmp_mixed_frames_right[i];
    }
  }
  kmeter_process(
   &mix_channel->kmeter_left, mix_channel->tmp_mixed_frames_left, start, end);
  kmeter_process(
   &mix_channel->kmeter_right, mix_channel->tmp_mixed_frames_right, start, end);
}

static inline void calc_channel_frames(
 struct channel *channel_ptr, jack_nframes_t start, jack_nframes_t end) {
  jack_nframes_t i;
  ...
  for (i = start; i < end; i++) {
    ...
    channel_ptr->prefader_frames_left[i - start] =
     channel_ptr->left_buffer_ptr[i];
    if (channel_ptr->stereo)
      channel_ptr->prefader_frames_right[i - start] =
       channel_ptr->right_buffer_ptr[i];
    ...
    if (channel_ptr->balance != channel_ptr->balance_new) {
      bal =
       channel_ptr->balance_idx * (balance_new - balance) / steps + balance;
    }
    float vol_l;
    float vol_r;
    if (channel_ptr->stereo) {
  ...
  kmeter_process(
   &channel_ptr->kmeter_left, channel_ptr->frames_left, start, end);
  if (channel_ptr->stereo)
    kmeter_process(
     &channel_ptr->kmeter_right, channel_ptr->frames_right, start, end);
}

static inline void mix(struct jack_mixer *mixer_ptr,
 jack_nframes_t start, /* index of first sample to process */
 jack_nframes_t end)   /* index of sample to stop processing before */
{
  GSList *node_ptr;
  ...
  for (node_ptr = mixer_ptr->input_channels_list; node_ptr;
       node_ptr = g_slist_next(node_ptr)) {
    channel_ptr = (struct channel *)node_ptr->data;
    calc_channel_frames(channel_ptr, start, end);
  }

  for (node_ptr = mixer_ptr->output_channels_list; node_ptr;
       node_ptr = g_slist_next(node_ptr)) {
    ...
    if (output_channel_ptr->system) {
      ...
      if (channel_ptr->stereo) {
        ...
        if (jack_port_connected(channel_ptr->port_left) == 0) continue;
      }
    }
...
static inline void update_channel_buffers(
 struct channel *channel_ptr, jack_nframes_t nframes) {
  channel_ptr->left_buffer_ptr =
   jack_port_get_buffer(channel_ptr->port_left, nframes);

  if (channel_ptr->stereo) {
    channel_ptr->right_buffer_ptr =
     jack_port_get_buffer(channel_ptr->port_right, nframes);
  }
}

#define mixer_ptr ((struct jack_mixer *)context)

static int process(jack_nframes_t nframes, void *context) {
  GSList *node_ptr;
  struct channel *channel_ptr;
#if defined(HAVE_JACK_MIDI)
  jack_nframes_t i;
  ...
  for (i = 0; i < event_count; i++) {
    ...
    if (in_event.size != 3 || (in_event.buffer[0] & 0xF0) != 0xB0 ||
        in_event.buffer[1] > 127 || in_event.buffer[2] > 127) {
      continue;
    }

    assert(in_event.time < nframes);

    LOG_DEBUG("%u: CC#%u -> %u", (unsigned int)(in_event.buffer[0]),
     (unsigned int)in_event.buffer[1], (unsigned int)in_event.buffer[2]);

    mixer_ptr->last_midi_channel = (int)in_event.buffer[1];
    channel_ptr = mixer_ptr->midi_cc_map[in_event.buffer[1]];

    /* if we have mapping for particular CC and MIDI scale is set for
    ...
    if (channel_ptr != NULL && channel_ptr->midi_scale != NULL) {
      if (channel_ptr->midi_cc_balance_index == (char)in_event.buffer[1]) {
        ...
        if ((mixer_ptr->midi_behavior == Pick_Up &&
          ...
          LOG_DEBUG(
           "\"%s\" balance -> %f", channel_ptr->name, channel_ptr->balance_new);
        }
      } else if (channel_ptr->midi_cc_volume_index == in_event.buffer[1]) {
        int current_midi =
         (int)(127 * scale_db_to_scale(channel_ptr->midi_scale,
        ...
        if ((mixer_ptr->midi_behavior == Pick_Up &&
          ...
          if (channel_ptr->volume_new != channel_ptr->volume) {
            channel_ptr->volume = interpolate(channel_ptr->volume,
             channel_ptr->volume_new, channel_ptr->volume_idx,
             channel_ptr->num_volume_transition_steps);
          }
          channel_ptr->volume_idx = 0;
          channel_ptr->volume_new = db_to_value(scale_scale_to_db(
           channel_ptr->midi_scale, (double)in_event.buffer[2] / 127));
          LOG_DEBUG(
           "\"%s\" volume -> %f", channel_ptr->name, channel_ptr->volume_new);
        }
        ...
        LOG_DEBUG(
         "\"%s\" out_mute %d", channel_ptr->name, channel_ptr->out_mute);
        ...
        LOG_DEBUG(
         "\"%s\" solo %d", channel_ptr->name, channel_is_soloed(channel_ptr));
      }
      channel_ptr->midi_in_got_events = true;
      if (channel_ptr->midi_change_callback)
        channel_ptr->midi_change_callback(
         channel_ptr->midi_change_callback_data);
    }
  }
  ...
  for (i = 0; i < nframes; i++) {
    for (cc_channel_index = 0; cc_channel_index < 128; cc_channel_index++) {
      ...
      if (channel_ptr->midi_out_has_events & CHANNEL_VOLUME) {
        ...
        if (midi_out_buffer == NULL) continue;

        midi_out_buffer[0] = 0xB0; /* control change */
        midi_out_buffer[1] = channel_ptr->midi_cc_volume_index;
        midi_out_buffer[2] =
         (unsigned char)(127 * scale_db_to_scale(channel_ptr->midi_scale,
                                value_to_db(channel_ptr->volume_new)));

        LOG_DEBUG("%u: CC#%u <- %u", (unsigned int)midi_out_buffer[0],
         (unsigned int)midi_out_buffer[1], (unsigned int)midi_out_buffer[2]);
      }
      if (channel_ptr->midi_out_has_events & CHANNEL_BALANCE) {
        ...
        midi_out_buffer[2] =
         (unsigned char)((channel_ptr->balance_new + 1.0) * 0.5 * 127);
      ...
      if (channel_ptr->midi_out_has_events & CHANNEL_MUTE) {
        ...
        midi_out_buffer[2] =
         (unsigned char)(channel_is_out_muted(channel_ptr) ? 127 : 0);
      ...
      if (channel_ptr->midi_out_has_events & CHANNEL_SOLO) {
        ...
        midi_out_buffer[2] =
         (unsigned char)(channel_is_soloed(channel_ptr) ? 127 : 0);
...
jack_mixer_t create(const char *jack_client_name_ptr, bool stereo) {
  (void)stereo;
  int ret;
  struct jack_mixer *mixer_ptr;
  int i;
  ...
  LOG_DEBUG(
   "Sample rate: %" PRIu32, jack_get_sample_rate(mixer_ptr->jack_client));

#if defined(HAVE_JACK_MIDI)
  mixer_ptr->port_midi_in = jack_port_register(mixer_ptr->jack_client,
   "midi in", JACK_DEFAULT_MIDI_TYPE, JackPortIsInput, 0);
  if (mixer_ptr->port_midi_in == NULL) {
    LOG_ERROR("Cannot create JACK MIDI in port");
    goto close_jack;
  }

  mixer_ptr->port_midi_out = jack_port_register(mixer_ptr->jack_client,
   "midi out", JACK_DEFAULT_MIDI_TYPE, JackPortIsOutput, 0);
...
close_jack:
  jack_client_close(
   mixer_ptr->jack_client); /* this should clear all other resources we obtained
...
void destroy(jack_mixer_t mixer) {
...
unsigned int get_channels_count(jack_mixer_t mixer) {
  return g_slist_length(mixer_ctx_ptr->input_channels_list);
}

const char *get_client_name(jack_mixer_t mixer) {
  return jack_get_client_name(mixer_ctx_ptr->jack_client);
}

int get_last_midi_channel(jack_mixer_t mixer) {
  return mixer_ctx_ptr->last_midi_channel;
}

unsigned int set_last_midi_channel(jack_mixer_t mixer, int new_channel) {
  mixer_ctx_ptr->last_midi_channel = new_channel;
  return 0;
}

int get_midi_behavior_mode(jack_mixer_t mixer) {
  return mixer_ctx_ptr->midi_behavior;
}

unsigned int set_midi_behavior_mode(
 jack_mixer_t mixer, enum midi_behavior_mode mode) {
  mixer_ctx_ptr->midi_behavior = mode;
  return 0;
}

jack_mixer_channel_t add_channel(
 jack_mixer_t mixer, const char *channel_name, bool stereo) {
  struct channel *channel_ptr;
  char *port_name = NULL;
  size_t channel_name_size;
  ...
  if (stereo) {
    ...
    channel_ptr->port_left =
     jack_port_register(channel_ptr->mixer_ptr->jack_client, port_name,
    ...
    channel_ptr->port_left =
     jack_port_register(channel_ptr->mixer_ptr->jack_client, channel_name,
      JACK_DEFAULT_AUDIO_TYPE, JackPortIsInput, 0);
    if (channel_ptr->port_left == NULL) {
      goto fail_free_channel_name;
    }
  ...
  channel_ptr->frames_left =
   calloc(MAX_BLOCK_SIZE, sizeof(jack_default_audio_sample_t));
  channel_ptr->frames_right =
   calloc(MAX_BLOCK_SIZE, sizeof(jack_default_audio_sample_t));
  channel_ptr->prefader_frames_left =
   calloc(MAX_BLOCK_SIZE, sizeof(jack_default_audio_sample_t));
  channel_ptr->prefader_frames_right =
   calloc(MAX_BLOCK_SIZE, sizeof(jack_default_audio_sample_t));
  ...
  channel_ptr->mixer_ptr->input_channels_list =
   g_slist_prepend(channel_ptr->mixer_ptr->input_channels_list, channel_ptr);
...
static jack_mixer_output_channel_t create_output_channel(
 jack_mixer_t mixer, const char *channel_name, bool stereo, bool system) {
  ...
  channel_ptr->tmp_mixed_frames_left =
   calloc(MAX_BLOCK_SIZE, sizeof(jack_default_audio_sample_t));
  channel_ptr->tmp_mixed_frames_right =
   calloc(MAX_BLOCK_SIZE, sizeof(jack_default_audio_sample_t));
  channel_ptr->frames_left =
   calloc(MAX_BLOCK_SIZE, sizeof(jack_default_audio_sample_t));
  channel_ptr->frames_right =
...
jack_mixer_output_channel_t add_output_channel(
 jack_mixer_t mixer, const char *channel_name, bool stereo, bool system) {
  struct output_channel *output_channel_ptr;
  struct channel *channel_ptr;

  output_channel_ptr =
   create_output_channel(mixer, channel_name, stereo, system);
...
void remove_channels(jack_mixer_t mixer) {
  GSList *list_ptr;
  for (list_ptr = mixer_ctx_ptr->input_channels_list; list_ptr;
       list_ptr = g_slist_next(list_ptr)) {
...
void kmeter_init(
 jack_mixer_kmeter_t kmeter, int sr, int fsize, float hold, float fall) {
  km->_z1 = 0;
  km->_z2 = 0;
  km->_rms = 0;
  km->_dpk = 0;
...
void kmeter_process(jack_mixer_kmeter_t kmeter, jack_default_audio_sample_t *p,
 int start, int end) {
  int i;
...
void remove_output_channel(jack_mixer_output_channel_t output_channel) {
  struct output_channel *output_channel_ptr = output_channel;
  struct channel *channel_ptr = output_channel;

  channel_ptr->mixer_ptr->output_channels_list =
   g_slist_remove(channel_ptr->mixer_ptr->output_channels_list, channel_ptr);
  free(channel_ptr->name);
...
void output_channel_set_solo(jack_mixer_output_channel_t output_channel,
 jack_mixer_channel_t channel, bool solo_value) {
  struct output_channel *output_channel_ptr = output_channel;

  if (solo_value) {
    if (g_slist_find(output_channel_ptr->soloed_channels, channel) != NULL)
      return;
    output_channel_ptr->soloed_channels =
     g_slist_prepend(output_channel_ptr->soloed_channels, channel);
  } else {
    if (g_slist_find(output_channel_ptr->soloed_channels, channel) == NULL)
      return;
    output_channel_ptr->soloed_channels =
     g_slist_remove(output_channel_ptr->soloed_channels, channel);
  }
}

void output_channel_set_muted(jack_mixer_output_channel_t output_channel,
 jack_mixer_channel_t channel, bool muted_value) {
  struct output_channel *output_channel_ptr = output_channel;

  if (muted_value) {
    if (g_slist_find(output_channel_ptr->muted_channels, channel) != NULL)
      return;
    output_channel_ptr->muted_channels =
     g_slist_prepend(output_channel_ptr->muted_channels, channel);
  } else {
    if (g_slist_find(output_channel_ptr->muted_channels, channel) == NULL)
      return;
    output_channel_ptr->muted_channels =
     g_slist_remove(output_channel_ptr->muted_channels, channel);
  }
}

bool output_channel_is_muted(
 jack_mixer_output_channel_t output_channel, jack_mixer_channel_t channel) {
  struct output_channel *output_channel_ptr = output_channel;
...
void output_channel_set_prefader(
 jack_mixer_output_channel_t output_channel, bool pfl_value) {
  struct output_channel *output_channel_ptr = output_channel;
  output_channel_ptr->prefader = pfl_value;
}

bool output_channel_is_prefader(jack_mixer_output_channel_t output_channel) {
  struct output_channel *output_channel_ptr = output_channel;
  return output_channel_ptr->prefader;
}

void output_channel_set_in_prefader(jack_mixer_output_channel_t output_channel,
 jack_mixer_channel_t channel, bool prefader_value) {
  struct output_channel *output_channel_ptr = output_channel;

  if (prefader_value) {
    if (g_slist_find(output_channel_ptr->prefader_channels, channel) != NULL)
      return;
    output_channel_ptr->prefader_channels =
     g_slist_prepend(output_channel_ptr->prefader_channels, channel);
  } else {
    if (g_slist_find(output_channel_ptr->prefader_channels, channel) == NULL)
      return;
    output_channel_ptr->prefader_channels =
     g_slist_remove(output_channel_ptr->prefader_channels, channel);
  }
}

bool output_channel_is_in_prefader(
 jack_mixer_output_channel_t output_channel, jack_mixer_channel_t channel) {
  struct output_channel *output_channel_ptr = output_channel;
